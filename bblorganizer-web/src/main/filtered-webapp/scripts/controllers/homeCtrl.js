...
angular.module('bblorganizer').controller('HomeCtrl', function ($scope, $http, $filter, ngTableParams, authenticationService) {
    ...
    $scope.vote = function(session) {
        $http.post('api/votes/vote', {idSessionMetadata: session.id}).success(function() {
    ...
    $scope.unvote = function(session) {
        $http.post('api/votes/unvote', {idSessionMetadata: session.id}).success(function() {
    ...
    $http.get('api/sessions/list').success(function(sessionMetadatas) {
        angular.forEach(sessionMetadatas, function(sessionMetadata) {
            // we will update local sessions with metadatas from the server
            var session = sessionsPerKey[sessionMetadata.baggerName+':'+sessionMetadata.title];
            if (session) {
                session.id = sessionMetadata.id;
                session.achievementDate = sessionMetadata.achievementDate;
                session.creationDate = sessionMetadata.creationDate;
                session.plannedDate = sessionMetadata.plannedDate;
                session.achieved = session.achievementDate ? new Date(session.achievementDate).getTime() < new Date().getTime() : false;

                // register this session per id
                sessionsPerId[session.id] = session;
            }
        });
        // and now will register all new sessions to the server
        var sessionsToRegister = [];
        angular.forEach(sessions, function(session) {
            if (!session.id) {
                // this is a new session, let's add it to the list to register
                sessionsToRegister.push({baggerName: session.baggerName, title: session.title});
            }
        });
        if (sessionsToRegister.length > 0) {
            // there are new sessions, let's say it to the server and wait for the new ids
            $http.post('api/sessions/create', sessionsToRegister, function(registeredSessions) {
                angular.forEach(registeredSessions, function(registeredSession) {
                    var session = sessionsPerKey[registeredSession.baggerName+':'+registeredSession.title];
                    session.id = registeredSession.id;
                    session.creationDate = registeredSession.creationDate;
                });
            });
        }
        // Now we can retrieve the votes (we assert here that new sessions will have no vote)
        $http.get('api/votes/list').success(function(votes) {
            angular.forEach(votes, function(vote) {
                var session = sessionsPerId[vote.idSessionMetadata];
                if (session) {
